...
class Message{
    ...
    /**
     * 消息撤回服务
     * @param $fromUserId   消息撤回用户id
     * @param $conversationType  消息类型 二人会话是 1 、讨论组会话是 2 、群组会话是 3
     * @param $targetId  目标 Id，根据不同的 ConversationType，可能是用户 Id、讨论组 Id、群组 Id。
     * @param $messageUID   消息唯一标识
     * @param $sentTime  消息发送时间
     * @return mixed
     */
   public function recallMsg($fromUserId,$conversationType,$targetId,$messageUID,$sentTime){
       ...
       try{
           ...
           if (empty($messageUID))
               throw new Exception('Paramer "messageUID" is required');

           if (empty($sentTime))
               throw new Exception('Paramer "sentTime" is required');
           ...
           $ret = $this->SendRequest->curl('/message/recall.json',$params,'urlencoded','im','POST');
           if(empty($ret))
               throw new Exception('bad request');
           return $ret;
